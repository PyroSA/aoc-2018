...
function orderInput (input) {
  ...
  return input.sort().reduce((acc, event) => {
    switch (event[19]) {
...
function getMaxTotalSleep(input) {
  const max = { duration: 0 }
  const guardTotal = input.reduce((acc, event) => {
    acc[event.guard] = (acc[event.guard] || 0) + event.duration;
    return acc;
  }, {})
  for (guard in guardTotal) {
    if (guardTotal[guard] > max.duration) {
      max.duration = guardTotal[guard];
      max.guard = guard;
    }
  }
  return max;
...
function getPeakMinute(input, guard) {
  const minutes = input.reduce((acc, event) => {
  ...
  const peak = minutes.reduce((acc, minute, index) => {
    if (minute > acc.max) {
      acc.max = minute;
      acc.minute = index;
    }
...
function strategy1 (input) {
  const max = getMaxTotalSleep(input);
  const peakMinute = getPeakMinute(input, max.guard);
